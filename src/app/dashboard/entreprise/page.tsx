...
export default function EntrepriseHubPage() {
  const { t } = useLocale();
  const { enterprises, deleteEnterprise, isLoading, pendingInvitations, respondToInvitation } = useEnterprise();
           ...
           {/* Section des invitations en attente */}
                        ...
                        {pendingInvitations.map(invite => (
                            <Card key={invite.id}>
                                <CardContent className="p-4 flex items-center justify-between">
                                    <p>Vous avez été invité à rejoindre <strong>{invite.name}</strong>.</p>
                                    <div className="flex gap-2">
                                        <Button variant="ghost" size="icon" className="text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={() => respondToInvitation(invite.id, 'declined')}><X className="h-5 w-5"/></Button>
                                        <Button variant="ghost" size="icon" className="text-primary hover:bg-primary/10 hover:text-primary" onClick={() => respondToInvitation(invite.id, 'accepted')}><Check className="h-5 w-5"/></Button>
                                    </div>
                                </CardContent>
                            </Card>
                        ))}
            ...
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <h1 className="text-2xl font-bold font-headline">Gestion des entreprises</h1>
                    <Button asChild>
                            ...
                            <Card key={enterprise.id} className="hover:border-primary/50 hover:shadow-lg transition-all">
                                <CardHeader className="flex flex-row items-center justify-between pb-2">
                                    <div>
                                        ...
                                        <CardDescription>{enterprise.members.find(m => m.uid === enterprise.ownerId)?.role || 'Propriétaire'}</CardDescription>
                                    </div>
