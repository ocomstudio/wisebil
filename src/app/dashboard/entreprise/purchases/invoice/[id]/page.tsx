// src/app/dashboard/entreprise/purchases/invoice/[id]/page.tsx
"use client";
...
const PurchaseInvoiceTemplate = React.forwardRef<HTMLDivElement, { purchase: Purchase }>(({ purchase }, ref) => {
        ...
        <Card className="max-w-4xl mx-auto my-8 shadow-2xl" ref={ref}>
            <CardHeader className="p-8" style={{ backgroundColor: `${brandColor}1A` }}>
                 <div className="flex justify-between items-start">
                    ...
                    <div className="text-right">
                        <h1 className="text-3xl font-bold font-headline" style={brandStyle}>{t('purchase_order_title_capital')}</h1>
                        <p className="text-muted-foreground"># {purchase.invoiceNumber}</p>
                        <Separator className="my-2" style={{ backgroundColor: brandColor, opacity: 0.5 }}/>
                        <p><span className="font-semibold">{t('issue_date_label')}:</span> {formatDateTime(purchase.date)}</p>
                    </div>
                </div>
            </CardHeader>
            <CardContent className="p-8">
                 <div className="flex justify-between items-start mb-8">
                    <div>
                        <h2 className="font-semibold text-muted-foreground mb-2">{t('supplier_label')}</h2>
             ...
             <CardFooter className="p-8 pt-0">
                 {/* Footer can be customized for purchase orders */}
             </CardFooter>
        </Card>
    );
});
...
export default function ViewPurchaseInvoicePage() {
  ...
  useEffect(() => {
    if (!isLoading && id) {
      const foundPurchase = getPurchaseById(id);
      setPurchase(foundPurchase || null);
    }
  }, [id, isLoading, getPurchaseById]);
  ...
  const handleDownload = async () => {
    ...
    try {
        ...
        pdf.save(`Bon_De_Commande-${purchase.invoiceNumber}.pdf`);

        toast({ title: t('download_success_title'), description: t('download_success_desc_purchase', { invoiceNumber: purchase.invoiceNumber}) });
    } catch (error) {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Bon de Commande ${purchase.invoiceNumber}`,
          text: `Voici le bon de commande pour ${purchase.supplierName}.`,
          url: publicUrl,
        });
      } catch (error) {
  ...
  if (!purchase) {
    return (
      <div className="p-4 md:p-8 text-center">
        <h2 className="text-2xl font-bold">{t('purchase_invoice_not_found_title')}</h2>
  ...
  return (
    <div>
        ...
        <PurchaseInvoiceTemplate purchase={purchase} ref={invoiceRef} />
    </div>
  );
}
