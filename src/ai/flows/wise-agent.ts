...
3.  **Identify ALL Financial Actions:** Do not miss a single action. You must capture everything from buying coffee to setting up a new savings plan. Each action MUST have all required fields (description, amount, category for transactions; name, amount, category for budgets; etc.).
...
6.  **Distinguish New vs. Existing (CRITICAL LOGIC):**
    ...
    - If the user says "ajoute 10000 à mon épargne 'Voiture'", and 'Voiture' is in the existing list, you MUST add it to the 'savingsContributions' array. The 'goalName' must EXACTLY match an existing goal name.
    - If the user mentions contributing to a savings goal that does NOT exist (e.g., "verse 5000 pour les 'Vacances'"), you MUST create it under 'newSavingsGoals' and set its 'currentAmount' to the contribution amount. Do not use 'savingsContributions' in this case.
